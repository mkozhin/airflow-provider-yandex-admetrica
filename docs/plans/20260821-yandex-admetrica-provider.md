...
- **Документация API** скачана в
  `/tmp/claude-1000/-home-mgcom-mkozhin-airflow-provider-yandex-admetrica/b56cdc11-fbcb-4052-a091-f47fae1f547a/scratchpad/`:
  `report_data.md` (метод `/v1/stat/data`), `getAllCampaigns.md`, `getCampaigns.md`,
  `getCampaign.md`, `authorization.md`, `reports-intro.md`, `param.md`,
  `attrandmetr_dim_all.md`, `attributes_events_{placements,audience,geo,browser,os,technology}.md`,
  `metrics_events_{basic,conversion,ecommerce,finance,video}.md`.
...
**Диагностика запросов в Loki** переносится из avito целиком, включая схему полей события,
маскирование токена и ограниченный слепок тела ответа — так существующие запросы и
дашборды Grafana работают с событиями нового провайдера без правок.
...
- Ключ внутри `dimensions` — базовое имя группировки: у `am:e:placement` это `placement`.
...
Плоский объект с полями management API без переименований: `campaign_id`, `name`,
`status`, `date_start`, `date_end`, `advertiser_id`, `advertiser_name`.
...
Таблица словаря кампаний: плоская схема по полям management API.
...
`request_method`, `request_url`, `request_params` (токен замаскирован), `request_headers`,
...
- [ ] дописать в `pyproject.toml` зависимости `apache-airflow>=2.9.1,<3.0` и `requests>=2.28`, `[project.optional-dependencies] dev = ["pytest>=7.0"]`, `[project.urls]` (Homepage, Documentation, Repository, Changelog, Issues)
...
- [ ] написать `tests/test_provider_info.py`: ключи присутствуют, `versions` не захардкожен, модули из `operators`/`hooks` импортируются
...
- [ ] написать тесты: обрезка по границе лимита, схлопывание пробелов и управляющих символов, маскирование короткого и длинного токена, вырезание токена из тела, тело в нечитаемой кодировке, тело без `charset`
...
- [ ] написать тесты: полнота набора ключей события, таблица уровней, тело прикладывается только выше `info`, `_record_exception` сохраняет первый тип, событие не отправляется при выключенном клиенте, разбор тела без объекта ошибки
...
- [ ] возвращать записи словаря с полями `campaign_id`, `name`, `status`, `date_start`, `date_end`, `advertiser_id`, `advertiser_name`
- [ ] написать тесты: одна страница, несколько страниц, `limit`/`offset` присутствуют в каждом запросе, отсчёт с 0, пустой список кампаний, отсутствующий ключ `campaigns`, архивные кампании присутствуют в результате, повторный вызов не делает второго запроса
...
- [ ] реализовать `_dimension_key(name)` — снятие префикса `am:e:` и приведение к базовому имени группировки
- [ ] реализовать `_metric_key(name, extra_params)` — снятие префикса `am:e:`, перевод в snake_case, подстановка фактического значения параметра вместо `<goal_id>` / `<currency>`
...
- [ ] написать тесты: группировка с `id` и без него, группировка с дополнительными полями, две строки с разным набором полей группировки, snake_case для `videoCompletePercent`, `am:e:goal<goal_id>Reaches` с `extra_params={"goal_id": 12345}`, `am:e:goal12345Reaches` без параметров, пустой `dimensions`, рассинхрон длины массива метрик
...
- [ ] реализовать пагинацию внутри кампании: `limit` (дефолт 10 000), **offset начинается с 1**, цикл до сбора `total_rows` либо до короткой страницы
- [ ] задавать детерминированный `sort` для многостраничных выборок: порядок строк между запросами документация не гарантирует, а сверка по количеству не отличит дубль от пропуска
...
- [ ] написать тесты: одна страница, несколько страниц, offset начинается с 1, `sort` присутствует в запросе, дубли строк между страницами обнаруживаются, расхождение с `total_rows` роняет таск, округлённый `total_rows` только предупреждает, превышение лимита метрик и группировок, кампания без данных, пустой `dimensions`, `sampled=true` даёт WARNING, `contains_sensitive_data=true` даёт WARNING, список кампаний запрашивается один раз
...
- [ ] реализовать `execute`: сбор статистики за день, запись файла, возврат списка `{kind: "stats", date, path}`; при нуле строк файл не создаётся и в результат ничего не добавляется
- [ ] написать тесты: путь формируется верно, JSONL пишется построчно, порядок ключей в записи детерминирован, строки с разным набором полей группировки пишутся без потерь, день без строк не создаёт файла, диагностика не конструируется при пустом `loki_conn_id`
...
- [ ] писать словарь в `{base_dir}/{safe_run_id}/{advertiser_id}/dict/campaigns/{snapshot_date}.json` и добавлять в результат запись `{kind: "dict", date: snapshot_date, path}`
- [ ] сохранять поля словаря без переименований, запись плоская
- [ ] написать тесты: словарь пишется в партицию даты выгрузки, `collect_dictionaries=False` его не создаёт, результат содержит обе записи `kind`, пустой список кампаний не создаёт файла, повторный запуск пишет тот же путь
...
- [ ] добавить `make_s3_params`, разводящий записи по `kind`: статистика в `{S3_PREFIX}/{advertiser_id}/stats/_year=…`, словарь в `{S3_PREFIX}/{advertiser_id}/dict/campaigns/_year=…`
...
- [ ] описать отдельную константу схемы и отдельную таблицу для словаря кампаний
- [ ] добавить загрузку в BigQuery через GCS по образцу avito, с явными схемами и без `autodetect`
- [ ] добавить `cleanup` с `trigger_rule="all_done"`, удаляющий локальную папку прогона
- [ ] снабдить DAG docstring-ом с описанием структуры, формата коннекшена и поведения при перезапуске отдельного дня
- [ ] написать `tests/test_example_dag.py`: DAG импортируется без ошибок, содержит ожидаемые задачи, `get_dates` возвращает даты по убыванию и включительно по границам, flatten разворачивает результаты нескольких map index, дедупликация оставляет один словарь
...
- [ ] перенести таблицы всех группировок из `attrandmetr_dim_all.md` и `attributes_events_*.md`: имя вида `am:e:placement`, русское название, описание, поддерживаемые операторы фильтрации, значение `since`
- [ ] сгруппировать группировки по разделам: размещения, аудитория, гео, браузер, ОС, технологии
- [ ] перенести таблицы всех метрик из `metrics_events_*.md`: базовые, конверсионные, ecommerce, финансовые, видео — имя, русское название, описание, тип, возможность фильтрации, значение `since`
- [ ] описать параметризацию из `param.md`: `goal_id` и `currency`, оба способа задания (в самом выражении и отдельным параметром запроса), связь с параметром оператора `extra_params`
...
- [ ] дать ссылку на первоисточник — документацию Яндекса — рядом с каждым разделом
- [ ] сверить перенесённые таблицы с исходными файлами документации на полноту
...
- [ ] описать принятые решения и их причины: отказ от `bytime`, запрос на каждую кампанию отдельно, выбор `getAllCampaigns` ради `total` и `advertiser_name`, вложенный JSON вместо плоских колонок, отсутствие фильтра кампаний по `date_start`/`date_end`, JSONL как единственный формат
