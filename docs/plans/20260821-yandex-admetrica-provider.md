...
| `total_rows` не служит условием останова | обход кончается на короткой странице: итог, заниженный округлением или устаревший на страницу, оборвал бы выгрузку на совпадении счётчиков и оставил хвост |
| Нечитаемый признак полноты роняет день | без целого `total_rows`, с небулевым `total_rows_rounded` или при расхождении точных итогов между страницами полноту нечем подтвердить, а короткий день прошёл бы незамеченным |
...
| `total_rows` нечитаем, `total_rows_rounded` не булев, два точных итога расходятся | `AirflowException` |
...
- [x] цикл пагинации до короткой страницы: `total` сверяет собранное, а не завершает обход; поле `campaigns` не список — `AirflowException`
...
- ➕ останов — только по короткой странице: `total` сверяет результат, а не завершает
  обход, иначе итог, совпавший на полной странице, оборвал бы список перед хвостом.
  `total` читается с каждой страницы, обязан быть целым и одним и тем же: и его
  отсутствие, и его смена между страницами роняют выгрузку
...
- [x] обрабатывать вырожденные случаи: значение группировки не объект, отсутствующее поле `name`, число метрик не совпадает с числом запрошенных — такая строка роняет день
...
- ➕ пустое значение доезжает до записи как `None` и пишется как JSON `null`: метрики
  типа `percents` и `currency` документированы пустыми там, где нет знаменателя или
  стоимости, поэтому отчёт из одних таких метрик отвечает строками из пустых чисел, и
...
- [x] останавливать обход **только** по короткой странице при любом значении `total_rows_rounded`, а `total_rows` держать критерием сверки: итог как критерий останова оборвал бы выгрузку на совпадении счётчиков (10 437 строк при `limit=10000` и округлённом вниз `total_rows=10000` дают полную первую страницу и потерянный хвост)
...
- [x] задать проекцию ключа явно: для каждой группировки берётся `id`, если под ним пришло значение, иначе `name`; кортеж объектов нехешируем, а проекция на один только `name` даёт ложный дубль на двух площадках с одинаковым названием и разными `id`
...
- ➕ флаг `total_rows_rounded` запоминается на всю кампанию: увидев его один раз, день
  сверяется мягко, иначе точное значение на следующей странице вернуло бы строгость, от
  которой отказ уже был принят
- ➕ останов — только по короткой странице: `total_rows` сверяет результат, а не
  завершает обход, иначе итог, заниженный округлением или устаревший на страницу,
  оборвал бы день на полной странице. `total_rows` и `total_rows_rounded` читаются с
  каждой страницы и обязаны быть читаемыми: нечитаемый итог, небулев флаг и расхождение
  точных итогов между страницами роняют день
...
- ➕ `id` со значением `null` читается как имя: спецификация гарантирует только `name`, и
  два разных значения группировки с пустым `id` — незарегистрированный креатив, площадка
  без номера — иначе спроецировались бы на один ключ, и второе из них уронило бы целый
  день как повтор
...
- [x] реализовать `test_connection() -> tuple[bool, str]` через запрос списка кампаний рекламодателя
- [x] реализовать ленивое кэширование соединения: `get_connection` вызывается ровно один раз за жизненный цикл хука независимо от числа запросов
- [x] написать тесты: успешная проверка, неуспешная проверка с текстом ошибки, `get_connection` вызывается один раз на множество запросов
- [x] запустить тесты — обязаны пройти до перехода к задаче 11
- ➕ текст неуспеха уходит через ту же точку маскирования, что и остальные каналы наружу
  (`_exception_text`), а искомое значение берётся из уже прочитанного соединения
  (`_maskable_token`), поэтому второго обращения к Airflow отказ не стоит
- ➕ проверка начинается с чтения соединения, чтобы отсутствующий `conn_id` назывался собой:
  дальнейший путь читает соединение best-effort и описал бы его отсутствие как extra без
  рекламодателя
...
- [x] дополнить `execute` выгрузкой словаря кампаний при `collect_dictionaries=True`
- [x] определять дату снапшота как дату выполнения выгрузки, а не как обрабатываемый день
- [x] писать словарь в `{base_dir}/{safe_run_id}/{advertiser_id}/dict/campaigns/{snapshot_date}.json` и добавлять в результат запись `{kind: "dict", date: snapshot_date, path, advertiser_id}`
- [x] сохранять поля словаря без переименований и проставлять `snapshot_date` в каждую запись файла — единственное место, где это поле появляется; та же дата именует файл и заполняется в `date` возвращаемой записи, поэтому колонка, путь и декоратор партиции BigQuery всегда согласованы
- [x] написать тесты: словарь пишется в партицию даты выгрузки, `snapshot_date` присутствует в каждой записи файла и равен дате в имени файла и в поле `date` результата, хук поля не проставляет, `collect_dictionaries=False` его не создаёт, результат содержит обе записи `kind` с `advertiser_id`, пустой список кампаний не создаёт файла, повторный запуск пишет тот же путь
- [x] запустить тесты — обязаны пройти до перехода к задаче 13
- ➕ дата снапшота берётся из старта прогона (`dag_run.start_date`), а не из часов машины:
  прогон, у которого дни разложены по map index, может пересечь полночь, и тогда часы дали
  бы две даты — два снапшота в двух партициях от одного прогона. При отсутствующем
  `dag_run.start_date` используется текущее время UTC
- ➕ словарь выгружается и в день без строк статистики: справочник кампаний от наличия
  показов не зависит, а пустой список кампаний файла не создаёт
...
- [x] написать DAG с параметрами `date_from` / `date_to` и задачей `get_dates`, возвращающей список дат по убыванию (от свежих в прошлое)
- [x] подключить `per_day.expand(day=dates)` с `max_active_tis_per_dag=1` у `collect`, чтобы дни шли последовательно
- [x] задать `max_active_runs=1`: `max_active_tis_per_dag` упорядочивает только экземпляры `collect` внутри прогона, а два одновременных прогона одного рекламодателя за тот же день пишут в один и тот же ключ S3 и в одну партицию BigQuery, и какой из них останется — вопрос порядка завершения
- [x] положить весь путь дня внутрь mapped task group `day`: сбор, обе выгрузки и загрузка в BigQuery едут одним map index, поэтому день грузится, пока соседний день падает, а параметры загрузки собираются из записей своего дня
- [x] грузить снапшот словаря один раз за прогон: отдельная группа после дней берёт первую запись `kind="dict"` из результатов дней и стоит с `trigger_rule="all_done"`, поэтому упавший день не мешает словарю уехать
- [x] добавить `make_s3_params`, разводящий записи по `kind`: статистика в `{S3_PREFIX}/{advertiser_id}/stats/_year=…`, словарь в `{S3_PREFIX}/{advertiser_id}/dict/campaigns/_year=…`; `advertiser_id` берётся из самой записи, а не из константы DAG
- [x] добавить загрузку в S3 через `LocalFilesystemToS3Operator` с `replace=True`
- [x] описать константу схемы BigQuery для статистики: `date` (DATE), `advertiser_id` (INTEGER), `campaign_id` (INTEGER), `dimensions` (JSON), `metrics` (JSON), партиционирование по `date`
- [x] описать константу схемы для словаря кампаний: `snapshot_date` (DATE), `campaign_id` (INTEGER), `name`, `status`, `date_start`, `date_end`, `advertiser_id` (INTEGER), `advertiser_name`; партиционирование по `snapshot_date`, отдельная таблица
- [x] добавить загрузку в BigQuery через GCS по образцу avito, с явными схемами и `autodetect=False`; партиция адресуется декоратором `table$YYYYMMDD` — по `date` для статистики, по `snapshot_date` для словаря, чтобы `WRITE_TRUNCATE` перетирал одну партицию, а не таблицу целиком
- [x] проверить, что зависимости из Task 1 покрывают импорты примера: `apache-airflow-providers-amazon` и `apache-airflow-providers-google` нужны тесту импорта и в CI
- [x] добавить `cleanup`, удаляющий локальную папку прогона, с `trigger_rule="none_failed"`: при `all_done` он сносит файлы и после окончательного отказа загрузки, и обычный ручной `clear` упавшей задачи повторить её уже не сможет — данные придётся выкачивать из API заново. Оставленная после сбоя папка стоит дискового места, потерянный день стоит перезапроса
- [x] снабдить DAG docstring-ом с описанием структуры, формата коннекшена и поведения при перезапуске отдельного дня
- [x] написать `tests/test_example_dag.py`: DAG импортируется без ошибок, содержит ожидаемые задачи, весь путь дня лежит внутри mapped task group, `get_dates` возвращает даты по убыванию и включительно по границам, снапшот словаря берётся один на прогон, правило жизненного цикла ограничено префиксом и добавляется к существующим, `max_active_runs=1` и `trigger_rule` у `cleanup` заданы как описано
- [x] запустить тесты — обязаны пройти до перехода к задаче 14
...
- [x] описать установку, требования (Python ≥3.10, Airflow ≥2.9.1 <3.0) и **необходимость тарифа Метрика Про** для доступа к API отчётов
- [x] описать настройку коннекшена: тип HTTP, токен в `password` без префикса `OAuth `, `extra` с `advertiser_id`; отдельно — коннекшн Loki для диагностики
- [x] описать быстрый старт с примером оператора и всеми параметрами
- [x] описать формат записи: служебные поля плоские, `dimensions` и `metrics` вложенными объектами, правила именования ключей, пример записи, схема таблиц BigQuery
- [x] описать раскладку файлов локально и в S3, hive-партиции, отсутствие `run_id` в ключе и перетирание дня
- [x] явно описать поведение при дне без данных: файл не создаётся, прежний файл в S3 остаётся нетронутым
- [x] описать поведение при смене набора `dimensions`/`metrics`: новые файлы пишутся в новой форме, ранее выгруженные остаются как есть, приведение истории — ручная перезаливка периода
- [x] описать отказы, ретраи и вид строк в логе таски; отдельно указать, что квоты и формат ошибок API не документированы, а значения `accuracy` в документации Яндекса не перечислены
- [x] описать диагностику в Loki: схему полей события, маскирование токена, поведение circuit breaker
- [x] дать ссылки на документацию AdMetrica: `https://yandex.ru/dev/admetrica/doc/ru/` и раздел группировок и метрик `https://yandex.ru/dev/admetrica/doc/ru/attrandmetr/dim_all`
- [x] написать `README.md` (EN) и `README_RU.md` (RU) с одинаковой структурой
- [x] проверить, что примеры кода из README запускаются как есть
- [x] запустить тесты — обязаны пройти до перехода к задаче 15
- ➕ проверка примеров вынесена в `tests/test_readme.py`: блоки кода обоих README
  разбираются, DAG-пример исполняется как есть, аргументы конструкторов сверяются с
  сигнатурами, таблица параметров оператора — с его сигнатурой и дефолтами, таблица
  именования ключей — с `_normalize_name`, список зарезервированных имён — с
  `_RESERVED_PARAMS`, а якорные ссылки — с заголовками; структура двух файлов
  сравнивается по уровням заголовков
...
- [x] скачать первоисточники заново на момент выполнения задачи — они живут по постоянным URL, и каждая страница доступна в markdown добавлением `.md` к адресу (например `https://yandex.ru/dev/admetrica/doc/ru/attrandmetr/dim_all.md`):
...
- [x] перенести таблицы всех группировок: имя вида `am:e:placement`, русское название, описание, поддерживаемые операторы фильтрации, значение `since`
- [x] сгруппировать группировки по разделам: размещения, аудитория, гео, браузер, ОС, технологии
- [x] перенести таблицы всех метрик: базовые, конверсионные, ecommerce, финансовые, видео — имя, русское название, описание, тип, возможность фильтрации, значение `since`
- [x] привести рядом с каждым именем ключ, под которым оно попадёт в запись: `am:e:deviceType` → `device_type`, `am:e:videoCompletePercent` → `video_complete_percent` — это и есть то, что аналитик пишет в `JSON_VALUE`
- [x] описать параметризацию: `goal_id` и `currency`, оба способа задания (в самом выражении и отдельным параметром запроса), связь с параметром оператора `extra_params` и список ключей, которые `extra_params` не принимает
- [x] описать документированные лимиты запроса: 20 метрик, 10 группировок, `limit` до 100 000, ограничения фильтра, разные значения `since` у группировок
- [x] указать, как эти имена передаются в параметры `dimensions` и `metrics` оператора, и привести готовые наборы для типовых отчётов
- [x] дать URL первоисточника рядом с каждым разделом
- [x] сверить перенесённые таблицы со скачанными страницами на полноту
- [x] дать ссылку на `docs/metrics-and-dimensions.md` из обоих README
- [x] запустить тесты — обязаны пройти до перехода к задаче 16
...
- [x] описать доменные термины: рекламодатель, кампания, группировка, метрика, словарь, снапшот словаря, служебные поля
- [x] описать архитектурные швы: `parse_connection`, `_map_row`, `_request_page`, `_build_path`, `_new_event`/`_emit_event`
- [x] описать правило санитизации идентификаторов и правила именования ключей в `dimensions`/`metrics`
- [x] описать политику работы с секретами: где живёт токен, где он маскируется, какие каналы (лог таски и события Loki) его никогда не получают
- [x] описать таблицу политик отказов по call-site
- [x] описать принятые решения и их причины: отказ от `bytime`, запрос на каждую кампанию отдельно, выбор `getAllCampaigns` ради `total` и `advertiser_name`, вложенный JSON вместо плоских колонок, отсутствие фильтра кампаний по `date_start`/`date_end`, JSONL как единственный формат, ключ строки отчёта как кортеж значений группировок, останов пагинации по флагу округления
- [x] запустить тесты — обязаны пройти до перехода к задаче 17
...
- [x] проверить, что все требования из Overview и Solution Overview реализованы
- [x] проверить граничные случаи: пустой день, пустой список кампаний, единственная кампания, кампания в архиве, расхождение `total_rows`, недоступный Loki, строки с разным набором полей группировки
- [x] проверить, что токен не попадает ни в лог таски, ни в события Loki ни при одном сценарии отказа
- [x] запустить полный набор тестов: `pytest tests/ -v`
- [x] проверить, что `pip install -e .` и `python -m build` отрабатывают без ошибок
- [x] проверить `python -c "from airflow_provider_yandex_admetrica import get_provider_info; print(get_provider_info())"`
- [x] проверить, что каждый модуль, объявленный в `get_provider_info()` в списках `operators` и `hooks`, импортируется — к этому моменту все они существуют
- ➕ приёмка нашла три канала наружу, обходивших единую точку маскирования, — все три
  закрыты и покрыты тестами:
  - `_log_report_caveats` писала `sample_share`, `sample_size`, `sample_space` и `data_lag`
    в лог таски прямо из ответа, тогда как в событие те же значения уходили через
    `_meta_value`. Теперь строка лога собирается тем же `_meta_value`
  - исключение о повторе строки внутри кампании подставляло в текст ключ строки, собранный
    из написанных сервером значений группировок. Добавлена `_describe_row_key`, пропускающая
    ключ через `_safe_text`
  - `_record_rate_limit` копировала заголовки `X-RateLimit-*` в событие только с обрезкой,
    без вырезания токена. Теперь значение заголовка проходит через `_scrub`
- ➕ сквозная проверка вынесена в `tests/test_token_never_leaves.py`: двенадцать сценариев
  отказа (401, 400, 403, 500 до исчерпания попыток, 429 с `Retry-After` и `X-RateLimit-*`,
  не-JSON тело на 200 и на 502, 200 без читаемых строк, две сетевые ошибки, оговорки отчёта,
  расхождение `total_rows`) прогоняются через `get_stats`, `get_campaigns` и
  `test_connection`, и по каждому проверяется, что ни в одной строке лога хука любого
  уровня, ни в одном событии Loki (дамп целиком), ни в тексте исключения не встречается ни
  токен, ни его середина

### Task 19: [Final] Обновление документации

- [x] зафиксировать в `CHANGELOG.md` версию первого релиза
- [x] сверить README с фактическим набором параметров оператора
- [x] запустить полный набор тестов после правок документации
- [x] перенести этот план в `docs/plans/completed/` (переносит харнесс после всех фаз)
- ➕ сверка README с сигнатурой `YandexAdmetricaStatsOperator.__init__` расхождений не нашла:
  обе таблицы параметров перечисляют те же пятнадцать имён в том же порядке, что и сигнатура,
  и называют те же значения по умолчанию
