...
- [ ] передать отбор в `hook.get_stats`; переписать первую строку докстроки
      класса (`operators/stats.py:130`, «Collect one day of statistics for every
...
- [ ] переписать `tests/test_operator.py:666` — `run.get_campaigns.assert_not_called()`
...
- [ ] обосновать это умолчание в `doc_md`: у примеров `schedule=None` и период
      ...
      `"active"`: там за период отвечает расписание
...
- [ ] переписать в `CONTEXT.md` раздел «Кампании не фильтруются ни по статусу,
      ни по датам» (строка 458): часть про `date_start`/`date_end` остаётся в
...
- [ ] добавить в `CONTEXT.md` секцию нового шва `CampaignSelection` в «Key
      seams» (строка 60) и три строки в таблицу «Error policies» (строка 394):
...
- [ ] добавить запись `0.4.0` в `CHANGELOG.md` — смена поведения по умолчанию и
