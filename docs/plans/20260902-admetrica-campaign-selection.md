...
Неразбираемую строку `literal_eval` отвергает `SyntaxError`, а он не подкласс
`ValueError`: `ast.literal_eval("[1, 2")` даёт `SyntaxError: '[' was never
closed`. Перехват берёт обе — `except (ValueError, SyntaxError)`, — а отказ
поднимается как `raise ValueError(...) from None`. Разрыв цепочки обязателен:
...
параметра. Id — положительные целые, они короткие и текста не несут, поэтому
цитируются как есть.
...
No campaign of the 127 listed is active (statuses seen: archived, paused); no statistics requested.
```
...
- [ ] перехватывать у `literal_eval` обе ошибки — `except (ValueError,
      SyntaxError)` — и поднимать отказ как `raise ValueError(...) from None`:
      `SyntaxError` не подкласс `ValueError` и держит исходную строку в
      атрибуте `text`, который traceback печатает целиком
...
- [ ] написать тест битой скобки с длинным значением: отказ — `ValueError`,
...
- [ ] переписать первую строку докстроки `get_stats`
      ...
      словарём
- [ ] сделать `_check_report_limits` и `_check_extra_params` публичными
      ...
      раньше `get_stats`
- [ ] добавить публичный `safe_text(value)` — обёртку над `_safe_text` с
...
- [ ] написать тест `safe_text`: значение с токеном соединения возвращается
...
- [ ] дозвать там же `check_report_limits(self.dimensions, self.metrics,
...
- [ ] провести каждое имя и каждый статус через `hook.safe_text` перед выводом —
...
- [ ] переписать `run.get_campaigns.assert_not_called()`
...
- [ ] написать тест: `campaign_names`, куда проставлено значение токена
...
- [ ] написать тест: `limit` вне 1…100 000 роняет таску `ValueError`, не
...
- [ ] записать в `doc_md` цену этого умолчания, названную прямо: DAG собирает
...
- [ ] назвать в обоих README границу умолчания `"active"` на регулярном
...
- [ ] неверно настроенный отчёт по-прежнему не стоит ни одного запроса: пустой
      `metrics` и `limit` вне 1…100 000 роняют таску раньше обхода кампаний
- [ ] ни имя кампании, ни статус не попадают в лог и в traceback минуя
