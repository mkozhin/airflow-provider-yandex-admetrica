...
Из этого следует правило: **в одном запросе задавайте одно написание, а не оба**.
Один ключ — это одна колонка, и два имени, нормализующихся в один ключ, пишут
одно значение: в записи остаётся последнее из них, провайдер пишет WARNING,
называющий оба имени их позицией в запросе, а одной из двух запрошенных метрик в
файле не будет. Так же
ведут себя и любые другие два имени, дающие один ключ.
...
переопределяет ни одного: параметры оператора выигрывают всегда. Четырнадцать имён
запрещены совсем — `ids`, `date1`, `date2`, `metrics`, `dimensions`, `preset`,
`limit`, `offset`, `sort`, `accuracy`, `include_undefined`, `filters`,
`timezone`, `lang`. Любое из них поднимает `ValueError` до первого запроса.
Каждое из них — это либо сам задаваемый вопрос, либо ответ на то, как он
задаётся: `preset` позволяет API самому задать метрики и группировки отчёта,
тогда как значения раскладываются по запрошенным именам по позиции, и цифры
легли бы под чужими ключами. У `filters`, `timezone`, `lang`, `accuracy` и
`include_undefined` есть отдельные параметры оператора, так что маршрут через
`extra_params` для них не нужен.
...
Три первых лимита оператор проверяет до того, как уйдёт первый запрос: список,
который вышел за границу, поднимает `ValueError` с указанием, какой это список и
на сколько он длиннее допустимого, а `limit` вне диапазона от 1 до потолка —
`ValueError` с полученным значением. Оператор запрашивает по 10 000 строк на
