...
она в отбор не попала бы, файла на неё никто не написал бы и ни один ключ S3 до неё не
дошёл бы — месяц приехал бы без неё. Адрес выгрузки доходит до дня одной кампании —
`_campaign_id=` последним уровнем партиции, — и перезалив части кампаний не достаёт до
данных остальных за те же дни. Поэтому цена этого умолчания — полнота заливки, а не
сохранность уже загруженного. У самого оператора умолчание другое, `"active"`: смысл
