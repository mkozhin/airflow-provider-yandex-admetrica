...
`campaign_scope` и `campaign_ids` — параметры прогона, оба доезжают до оператора
шаблоном и решают, по каким кампаниям кабинета идёт обход статистики. Справочник это
не затрагивает: снапшот остаётся полным снимком кабинета при любом отборе, и по нему
потом видно, какая кампания в выгрузку не попала.

Умолчание примера — `campaign_scope="all"`. У DAG `schedule=None`, а период по умолчанию —
последние тридцать дней: штатный прогон здесь разовая заливка месяца назад. Кампания,
ушедшая в архив внутри этого месяца, за его дни отдала настоящие строки; при `"active"`
она в отбор не попала бы, файла на неё никто не написал бы и ни один адрес в обеих
витринах до неё не дошёл бы — месяц приехал бы без неё. Адрес выгрузки доходит до дня
одной кампании в обеих витринах — `_campaign_id=` последним уровнем партиции в S3 и
таблица `stats_{advertiser_id}_{campaign_id}` в BigQuery, — и перезалив части кампаний не
достаёт до данных остальных за те же дни. Поэтому цена этого умолчания — полнота заливки,
а не сохранность уже загруженного. У самого оператора умолчание другое, `"active"`: смысл
отбора там — не тратить запрос на кампанию, которая сегодня уже не крутится.

Ручной запуск задаёт отбор в форме. `campaign_ids` перечисляет кампании через запятую
(`1234, 5678`) и перекрывает `campaign_scope` целиком: названную кампанию соберут,
архивную в том числе. Так добирают день одной кампании, не трогая остальные, и день
стоит тогда одного запроса вместо запроса на каждую кампанию кабинета. Пустое поле
означает «не задано», и отбор решает `campaign_scope`.

`campaign_scope="active"` в форме уместен на узком свежем окне, где активные кампании
кабинета и есть те, что отчитываются за собираемые дни. На перезаливке прошлого
периода он режет выгрузку: за те дни отдают строки и кампании, закрытые к сегодняшнему
дню.

Цена умолчания `"active"` на регулярном расписании названа прямо. DAG собирает день D,
запускаясь D+1, а статус кампании читается на момент прогона: кампания, закончившаяся
в конце D и уведённая в архив к запуску, отбор не проходит — её последний и самый
полный день не собирается. Лог этого не выделит: пропущенные кампании при `"active"` —
норма, а не сигнал. Окно потери равно лагу расписания, добор — ручной запуск с
`campaign_ids` этой кампании.
...
@dag(
    ...
    params={
        ...
        "campaign_scope": Param(
            "all",
            type="string",
            enum=["active", "all"],
            description="Какие кампании обходить: active — только активные, all — все",
        ),
        "campaign_ids": Param(
            "",
            type="string",
            description="id кампаний через запятую; пусто — отбор по campaign_scope",
        ),
    },
...
def admetrica_to_bq_and_s3():
    ...
    def per_run_dictionary(mapped_results) -> None:
        ...
        выгрузки, создание таблицы и загрузка остаются декларативными операторами.
        """
