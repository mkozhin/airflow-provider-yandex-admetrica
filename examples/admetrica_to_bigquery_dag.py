...
она в отбор не попала бы, файла на неё никто не написал бы и ни одна таблица под неё не
завелась бы — месяц приехал бы без неё. Адрес загрузки доходит до дня одной кампании —
таблица `stats_{advertiser_id}_{campaign_id}`, партиция дня, — и перезалив части кампаний
не достаёт до данных остальных за те же дни. Поэтому цена этого умолчания — полнота
заливки, а не сохранность уже загруженного. У самого оператора умолчание другое,
`"active"`: смысл отбора там — не тратить запрос на кампанию, которая сегодня уже не
крутится.
