...
- `apache-airflow-providers-amazon` for the S3 destination and `apache-airflow-providers-google>=13.0.0` for the BigQuery one, which this package installs only under its `dev` extra — a deployment running a DAG installs them itself. The floor on the google provider is what the campaign's table is created by: `BigQueryHook.create_table` is the method 13.0.0 carries, and the constraint set of Airflow 2.9.1 pins 10.17.0, so a deployment on that set installs the google provider above its own constraints or the day's load fails on the first campaign
