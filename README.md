...
**A campaign is `active` or `archived`.** Those are the two words [the campaign list](https://yandex.ru/dev/admetrica/doc/ru/openapi/management/Kampanii/getAllCampaigns) documents for `status`, and `campaign_scope="active"` keeps the first of them, comparing case and surrounding space aside. An answer wording a running campaign outside those two would match nothing and export an empty day while the task stays green, since an empty scope is a warning by design. That WARNING is where such an answer shows: it names the statuses it did meet and holds up both readings — an advertiser whose campaigns are all over, and a vocabulary that has moved. Compare the statuses it names with what the interface shows, and collect the day with `campaign_scope="all"` until they agree.
...
- [The campaign list](https://yandex.ru/dev/admetrica/doc/ru/openapi/management/Kampanii/getAllCampaigns) — the method behind `get_campaigns()`, and the two words it documents for `status`
